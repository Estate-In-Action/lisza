...
- [x] **Cost centers / accounting dimensions** — tag postings by project/department/class for dimensional reporting. `[Finance-first]` *(extends: budgeting notes "project/department dimensions remain later")* — ERPNext, LedgerSMB. *(Shipped 2026-07-12 CR-009: `cost_centers.py` — `cost_centers` registry + `split_dimensions` sidecar overlay (core splits untouched, posts nothing); post-hoc `tag_split`/`tag_entry`, soft-delete, `dimension_report` + `cost_center_pnl` with (unassigned) bucket. `cost_center_post`/`cost_center` API modes + Cost Centers section in `/lisza/workspace`. 17 tests; full suite 338; live-verified on jb-design.)*
- [x] **Period / fiscal-year closing** — closing vouchers that roll income & expense into retained earnings and lock the period. `[Finance-first]` *(extends: GL note "closing workflows remain a later extension")* — ERPNext, LedgerSMB. *(Shipped CR-008: `period_close.py`, balanced closing entry to retained-earnings 300 + period lock guard on post; preview-then-close UI. 14 tests; live close verified on harborside-group 2026-01.)*
